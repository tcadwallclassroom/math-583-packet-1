...
Choose a different data set (not 'county' or any of the variants of 'county', and different from any of your classmates) from one of the openintro or tidyverse packages. You can see a list of datasets using the command `data()`. Be sure to use the `help()` command to get more information on your data set; you will need at least 2 numerical and one categorical variable in your data set, and ideally, at least 50 rows (data points.) 

Then, create plots comparing two variables with the following characteristics:
...
1. A dot-and-line chart showing the maximum, minimum, and median of a numerical variable, separated into groups using a categorical variable.

You may need to use `filter()`, `mutate()`, or `na.omit()` to create variables or narrow down your data set. 
